...
// #include <time.h>   // Do inicjalizacji generatora liczb losowych (problematyczne na AVR)
...
// Piny dla segmentów (dla wspólnej katody - CC)
// gfedcba (bit 7 dla DP, jeśli używany, tutaj 0)
...
volatile uint8_t disp_financial_d3 = 0; // Trzecia cyfra od lewej
...
void initialize_ports(void) {
    ...
    SEGMENT_DDR |= 0xFF; // Ustaw PD0-PD6 jako wyjścia (A-G)
    // Jeśli używasz DP na PD7: SEGMENT_DDR |= (1 << PD7);

    // Piny wyboru cyfry (PORTC PC0-PC3 jako wyjścia)
    DIGIT_SELECT_DDR |= DIGIT_ENABLE_MASK;
    DIGIT_SELECT_PORT &= ~DIGIT_ENABLE_MASK; // Domyślnie wyłącz wszystkie cyfry (stan LOW)

    // Klawiatura - rzędy (PORTB PB0-PB3 jako wyjścia, domyślnie wysoki stan)
    KEYPAD_ROWS_DDR |= 0x0F; // Piny PB0-PB3 jako wyjścia
    KEYPAD_ROWS_PORT |= 0x0F; // Ustaw domyślny stan wysoki dla PB0-PB3
 ...
 */
void set_segment_data(uint8_t digit_value) {
    SEGMENT_PORT = segment_map_common_cathode[digit_value];
}
 ...
 */
void select_digit_enable(uint8_t digit_pin) {
    // Wyłącz wszystkie cyfry najpierw
    DIGIT_SELECT_PORT &= ~DIGIT_ENABLE_MASK; // Wyłącz wszystkie cyfry (ustaw na LOW)

    // Aktywuj wybraną cyfrę (ustaw na HIGH)
...
void display_update_task(void) {
    ...
    SEGMENT_PORT = 0x00; // Upewnij się, że wszystkie segmenty są wyłączone zanim zmienisz digit enable
    DIGIT_SELECT_PORT |= DIGIT_ENABLE_MASK; // Wszystkie cyfry OFF

    // Przechodzenie przez wszystkie 4 cyfry
    switch (current_digit_index) {
        case 0: // Cyfra 1 (D1) - nieużywana w tej grze
            digit_value_to_display = 0; // Zawsze 0
            digit_enable_pin = DIGIT_1_ENABLE_PIN;
            break;
        case 1: // Cyfra 2 (D2) - nieużywana w tej grze
            digit_value_to_display = 0; // Zawsze 0
            digit_enable_pin = DIGIT_2_ENABLE_PIN;
            break;
        case 2: // Cyfra 3 (D3) - setki/dziesiątki (w trybie finansowym)
            ...
            break;
        case 3: // Cyfra 4 (D4) - jednostki (w obu trybach)
            if (display_mode == 0) { // Tryb finansowy
    ...
    // Wyłącz wszystkie cyfry, ustaw segmenty, włącz wybraną cyfrę
    set_segment_data(digit_value_to_display); // Ustaw segmenty dla bieżącej cyfry
    select_digit_enable(digit_enable_pin); // Włącz bieżącą cyfrę
...
void run_animation(void) {
    ...
    SEGMENT_PORT = 0x00; // Wyłącz wszystkie segmenty
    if (animation_frame_counter % 2 == 0) { // Miganie co 2 klatki (szybkie)
        SEGMENT_PORT |= SEG_G_MASK; // Włącz segment G
    } else {
        //SEGMENT_PORT &= ~SEG_G_MASK; // Wyłącz segment G
    }

    // Pozostałe segmenty OFF
    //SEGMENT_PORT &= ~0x3F; // Wyłącz segmenty A-F (maska 0b00111111)

    // Aktywuj wszystkie cyfry jednocześnie dla animacji (możesz też je multipleksować)
    DIGIT_SELECT_PORT |= DIGIT_ENABLE_MASK; // Włącz wszystkie cyfry
}
...
uint8_t read_keypad(void) {
    ...
    for (uint8_t row_idx = 0; row_idx < 4; row_idx++) {
        KEYPAD_ROWS_PORT = 0x0F; // Wszystkie PB0-PB3 na HIGH (nieaktywne)
        KEYPAD_ROWS_PORT &= ~(1 << keypad_row_pins[row_idx]); // Wybrany pin rzędu na LOW
        _delay_us(10);

        uint8_t col_data_raw = KEYPAD_COLS_PIN & 0x0F; // Odczytujemy tylko PE0-PE3

        if (~col_data_raw & 0x0F) {
            for (uint8_t col_idx = 0; col_idx < 4; col_idx++) {
                if (!((col_data_raw >> col_idx) & 1)) {
                    key_pressed = (row_idx * 4) + col_idx + 1;
                    _delay_ms(50); // Debouncing
                    while (!((KEYPAD_COLS_PIN >> col_idx) & 1)) {
                        _delay_ms(10);
                    }
                    KEYPAD_ROWS_PORT |= 0x0F; // Przywróć rzędy do HIGH
                    return key_pressed;
                }
            }
        }
...
void switch_display_mode(void) {
    ...
    // Upewnij się, że segmenty są wyłączone po animacji
    SEGMENT_PORT = 0x00;
...
int main(void) {
    ...
    update_display_values();
    // NIE wywołuj _delay_ms() tuż po starcie, to może zablokować multipleksowanie.
    // Zmienna animation_active powinna być domyślnie 0.
    animation_active = 0; // Upewnij się, że zaczyna jako false
        ...
        if (timer_flag) {
            ...
            switch (pressed_key) {
                ...
                case 10: // Szybka gra
                    ...
                    break;
                case 11: clear_bet(); update_display_values(); break;
